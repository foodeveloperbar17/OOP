...
public class Piece {
	...
	*/
	...
	private void initSkirt(TPoint[] points){
		skirt = new int[width];
		Arrays.fill(skirt, -1);
		for(TPoint curr: points){
			if(skirt[curr.x] == -1){
				skirt[curr.x] = curr.y;
			} else if(skirt[curr.x] > curr.y){
				skirt[curr.x] = curr.y;
			}
		}
	}

	private void calculateShape(TPoint[] points){
		for(TPoint curr: points){
			if(width < curr.x){
				width = curr.x;
			}
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		for (int i = 0; i < body.length; i++) {
			TPoint old = body[i];
			TPoint newPoint = new TPoint(height - 1 - old.y, old.x);
			newBody[i] = newPoint;
		}
	...
	*/
	public boolean equals(Object obj) {
		...
		for (int i = 0; i < body.length; i++) {
			if(!containsPoint(body[i], other.body)){
				return false;
			}
		}
		return true;
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece prev = root;
		while(true){
			Piece curr = prev.computeNextRotation();
			if (curr.equals(root)) {
				prev.next = root;
				break;
			}
